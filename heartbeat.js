    ...
    async sendHeartbeat(instance) {
        ...
        try {
            ...
            if (response.statusCode === 200) {
                ...
                let newCsrfToken = instance.csrfToken
                if (response.headers['set-cookie']) {
                    const csrfCookie = response.headers['set-cookie'].find(cookie => cookie.startsWith('csrfCookie='))
                    if (csrfCookie) {
                        newCsrfToken = csrfCookie.split(';')[0].split('=')[1]
                    }
                }
                // Return the new auth header and CSRF token
                return { authHeader: newAuthHeader, csrfToken: newCsrfToken }
            } else {
                ...
                if (response.statusCode === 401) {
                    ...
                    if (authResult) {
                        return { authHeader: instance.authHeader, csrfToken: instance.csrfToken }
                    }
                }
            }
        } catch (error) {
